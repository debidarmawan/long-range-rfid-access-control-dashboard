...
/**
 * Module for build datatables
 * @module
 * @param {object} options refer to README.md for a list of properties
 * @return {object}
 */
module.exports = function (options) {
	...
	/**
	 * Build an optional "USE sDatabaseOrSchema" for MySQL / Postgres or "ALTER SESSION SET CURRENT_SCHEMA = sDatabaseOrSchema" statement for Oracle if sDatabaseOrSchema is set
	 * @private
	...
	/**
	 * Build the date partial that is used in a WHERE clause
	 * @private
	 * @return {*}
	 */
	...
	/**
	 * Build a complete SELECT statement that counts the number of entries
	 * @private
	 * @param {object} requestQuery If specified then produces a statement to count the filtered list of records Otherwise the statement counts the unfiltered list of records
	...
	/**
	 * Build the WHERE clause otherwise uses aoColumnDef mData property
	 * @private
	...
	/**
	 * Builds the search portion of the WHERE clause using LIKE (or ILIKE for PostgreSQL)
	 * @private
	...
	/**
	 * Builds an array of LIKE / ILIKE statements to be added to the WHERE clause
	 * @private
	 * @param {object} {requestQuery} The datatable parameters that are generated by the client
	...
	/**
	 * Builds the search portion of the WHERE clause using ILIKE
	 * @private
	...
	/**
	 * Builds the search portion of the WHERE clause using LIKE
	 * @private
	 * @param {string} colName The column to search
	 * @param {string} searchVal The value to search for
	 ...
	 * @private
	 * @param {object} requestQuery The Datatable query string (we look at sort direction and sort columns)
	 * @return {string} The ORDER clause
	 */
	function buildOrderingPartial(requestQuery) {
		var orderQuery = [requestQuery.order[0]]
		var query = [];
		var l = _u.isArray(orderQuery) ? orderQuery.length : 0;
		for (var fdx = 0; fdx < l; ++fdx) {
			var order = orderQuery[fdx],
	 ...
	 * @param {object} requestQuery The Datatable query string (we look at length and start)
	 ...
	 * @param {query} queryResult An array of SQL response objects, each of which must, in order, correspond with a query string returned by buildQuery
	...
	/**
	 * Extract the value from a database response
	 * @private
